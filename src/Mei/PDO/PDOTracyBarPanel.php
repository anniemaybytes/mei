...
final class PDOTracyBarPanel implements IBarPanel
{
    ...
    {
        if (!$this->queries) {
            $this->queries = $this->pdo->getLog();
        }

        $html = '<img src="' . $this->icon . '" alt="PDO queries" /> ';
        ...
        return $html . ' / ' . number_format(array_sum(array_column($this->queries, 'time')), 2, '.', ' ') . ' ms';
    }
